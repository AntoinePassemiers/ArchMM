...
from typing import List, Union, Any, Tuple, Sized, Collection
...
cdef class HMM:
    ...
    def __init__(self, n_states: int, n_features: int = 1):
    ...
    def fit(self, data: Any, max_n_iter: int = 100):
        ...
        cdef data_t[:, :] log_alpha = np.zeros((n, self.n_states), dtype=py_data_t)
        cdef data_t[:, :] log_beta = np.zeros((n, self.n_states), dtype=py_data_t)
        cdef data_t[:, :] log_gamma = np.zeros((n, self.n_states), dtype=py_data_t)
        cdef data_t[:, :, :] log_xi = np.zeros((n - 1, self.n_states, self.n_states), dtype=py_data_t)
        cdef data_t[:] tmp = np.zeros(self.n_states, dtype=py_data_t)

        cdef int t, i, j, k, w, z, best_k
        cdef data_t lse, forward_ll, backward_ll

        for _ in range(max_n_iter):

            self.emission_log_prob(data, np.asarray(log_b))
